...
server.get('/api/checkout', function(req, res) {
	...
	if (stop > previousRequestTime + 3000) {
		...
		var passwordLogItem = {
			'cabinetID': cabinetItem.id,
			'oldPassword': cabinetItem.oldPassword,
			'newPassword': cabinetItem.newPassword,
			'timeStamp': moment().format()
		}
		data.passwords.push(passwordLogItem);

		passwordsRef.push(passwordLogItem);

		//console.log(JSON.stringify(cabinetItem));
...
//		fs.writeFileSync('logItems_updated.json', JSON.stringify(data, null, 2), 'utf-8');
...
//data = JSON.parse(fs.readFileSync('logItems_updated.json', 'utf8'));
